...
// const PackageAccordion = ({ packageData,user, quotation }) => {
...
const Customization = ({ quotation }) => {
  ...
  useEffect(() => {
    if (quotation) {
      ...
      // If we have a quotation, skip package selection step
      if (pkg) setActiveStep(1);
    }
  }, [quotation, packages]);

  const handlePackageSelect = (pkg) => {
    setSelectedPackage(pkg);
    setActiveStep(1); // Move to next step after package selection
  };
    ...
    <div className="p-4">
      ...
      >
        ...
        <Stepper.Step
          ...
          description="Choose your preferred package"
        >
          <div className="py-6">
        ...
        {/* Steps 1-5: Package Customization */}
        <Stepper.Step
          label="Event details"
          description="Provide event date, venue, and guest count"
        >
...
// Update PackageAccordion to accept stepper props and conditionally render content
const PackageAccordion = ({ packageData, user, quotation, stepperProps }) => {
  const {
    activeStep,
    nextStep,
    prevStep,
    isFirstStep = false,
    isCustomizeStep = false,
    isMenuStep = false,
    isAddOnsStep = false,
    isReviewStep = false,
  } = stepperProps || {};

  // ... (keep all your existing PackageAccordion state and logic)

  // Modify the return to conditionally render based on the step
  return (
    <div>
      {/* Only show package customization when it's the customize step */}
      {isCustomizeStep && (
        <div className="grid grid-cols-3">
          <Accordion>
            {bundles.map((bundle) => (
      ...
      {/* Only show data form when it's the first step */}
      {isFirstStep && (
        <DataForm
          requestedDate={requestedDate}
          setRequestedDate={setRequestedDate}
      ...
      {/* Only show food form when it's the menu step */}
      {isMenuStep && (
        <FoodForm
          groupedFoods={groupedFoods}
          setGroupedFoods={setGroupedFoods}
      ...
      {/* Only show add-ons when it's the add-ons step */}
      {isAddOnsStep && (
        <div className="grid grid-cols-3 gap-2">
          <div className="col-span-2">
            <AddOnsForm
      ...
      {/* Only show review when it's the review step */}
      {isReviewStep && (
        <div className="grid grid-cols-2 gap-4">
      ...
      <Group position="apart" mt="md">
        ...
        <Button
          onClick={isReviewStep ? handleSubmitQuotation : nextStep}
          loading={loading}
        >
          {isReviewStep ? "Submit Quotation" : "Next"}
        </Button>
      </Group>
    </div>
  );
...
// Keep all your other components (AddOnsForm, FoodItem, CategorySelect, FoodForm, DataForm) exactly as they are
